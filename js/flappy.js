...
// function telaInicio (){
// const inicio = novoElemento('div', 'inicio')
// const botaoPlay = novoElemento('button','btn-play')

// }

function Barreira(reversa = false){ // false barreira normal. borda-corpo
    this.elemento = novoElemento('div','barreira')
    const borda = novoElemento('div','borda')
    const corpo = novoElemento('div','corpo')
    this.elemento.appendChild( reversa ? corpo : borda) // reversa, começa pelo corpo da barreira 
    this.elemento.appendChild( reversa ? borda : corpo) 
    this.setAltura = altura => corpo.style.height = `${altura}px` //recebe altura como string // visivel em outras funções
}   
...
function ParDeBarreiras(altura,abertura,x){
    ...
    this.inferior = new Barreira(false) // barreira normal
...
// const b = new ParDeBarreiras(700,300,500)
...
function colidiu (passaro,barreiras){
    ...
    barreiras.pares.forEach(ParDeBarreiras => {
        ...
        // else if(colidiu == true) windows.reload()
    })
         return colidiu 
}

function repetirJogo(){
    const botao = document.querySelector('.btn-repetir')
